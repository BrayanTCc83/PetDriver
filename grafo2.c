...
int dfs_topol_traverse(Graph* g, Vertex* v, int time, Queue *list)
{
   ...
   time++;
   Vertex_SetFinished_time(v, time);
   Vertex_SetColor(v, WHITE);
   Queue_Enqueue(list, &v);
...
data[i-1].val = i * 100; data[i-1].name = name;
}
printf( "Data table:\n" );
for( size_t i = 0; i < MAX_DATA; ++i ) {
      printf( "[]=%ld, .val=%d, .name=%c\n",
            i,
            data[i].val,
            data[i].name );
    }
...
Graph* grafo = Graph_New(
    ...
    eGraphType_DIRECTED ); // será un grafo dirigidofor( size_t i = 0; i < MAX_DATA; ++i ) {
...
Graph_AddEdge(grafo, 600, 900);
Graph_AddEdge(grafo, 700, 300);
Graph_AddEdge(grafo, 700,1000);
Graph_AddEdge(grafo, 1000,600);
Graph_AddEdge(grafo, 900, 800);
Graph_AddEdge(grafo, 800, 400);
